...
class ParsingBase {
    ...
    class func dislodgeAnnotaion(content:String) -> String {
        ...
        var newStr = ""
        newStr = regexLine.stringByReplacingMatches(in: content, options: NSRegularExpression.MatchingOptions(rawValue:0), range: NSMakeRange(0, content.count), withTemplate: Sb.newLine)
